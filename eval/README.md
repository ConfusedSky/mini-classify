...
* `eval/rig.py` holds every `OffscreenRenderer` for the process lifetime and
  **never destroys one** — Filament aborts from that destructor (CLAUDE.md).
  ...
  example.
* Ground truth is `../labels/up_axis_labels.json`, loaded through
...
| `geo_floor.py` | Tests scaling geometry's vote by how much print-base evidence it actually has (`w = min(1, best/floor)**p`) against the pre-v3 unweighted min-max. **Answered 2026-09-15 on 214 models: the shipped `GEO_FLOOR_POWER = 2` is +6 over pre-v3 (201/214 vs 195/214) and escalates less (18 vs 22), monotone in p, and a hard switch does no better.** On the 49 models it was first asked against it fixed one model and `orig`/`holdout` still do not move — every gain is in the newer sets. Rows are derived from the label file and the backbone from the cache's run-params, after a run that scored `patch14-384` against an `embed-cache512` built with `patch16-512` and reported 47 of 214 models. Distinct from the absolute-scaled scheme in the retired `ensemble.py`, which replaced min-max and lost. LEARNINGS 2026-09-15. |
| `pose_baseline.py` | The production pose tiers over **any** label set — geometry, SigLIP, the min-max ensemble, the margin and whether the gate fires — written as the input the arbiter harnesses score against (`--baselines`). Exists because `load_baselines()` reads a 44-model file and both VLM harnesses filter to it, so a 206-label run scored 44 and ignored 162. Reads its backbone from the cache's run-params, not `rig.embedder()`'s default, which is a *different* tower and moves low-margin picks. LEARNINGS 2026-09-10. |
...
| `renderer_pilot/` | Does the renderer move SigLIP **search** quality? Six render arms × two prompt arms × 12 queries over 301 models of embed-cache512, judged by a VLM (gemini-3.6-flash, cross-checked by GLM-5.3-Flash, κ 0.80): every alternative beats production by +0.015–0.026 nDCG@20, uniformly and within noise. Own README. LEARNINGS 2026-08-29. |
| `pose_label_calibration.py` | What is a labeller's proposal worth? Renders the **already-labelled** models as the same six-tile sheets the arbiter sees, into a manifest carrying no label and no geometry column, with the answers written to a separate `truth.json` that only `--score` opens. Run before letting anyone's proposals seed a labelling page — the arbiter tables put gemini-3.5-flash at 43/44 and haiku at 20/44 on these same sheets, so "a model looked at it" is not a quality claim. LEARNINGS 2026-09-09. |
| `pose_label_expand.py` | Widens the labelled set: samples the walk excluding every path already labelled (and, with `--exclude`, an earlier batch), renders six-tile sheets, builds the labelling page, and merges confirmed picks into `../labels/up_axis_labels.json` under a new set name. Two page modes because the questions cost their reader differently — `--triage` asks only *model or loose part?* (**half** of an unfiltered draw, measured), then the axis pass runs over what survives with a proposal pre-selected; a proposal the labeller flagged is shown with nothing selected. `LIKELY_PART` and `lettered_family` skip likely parts at draw time (`--all-shapes` disables), and a page re-applies both — plus the collection vocabulary — to a batch drawn before they existed. What lands in the labels file is what the human pressed. `--mode {axis,parts,triage}` picks the question and `--note` prints a banner above the cards, for a pass that reads `u` differently or carries a convention. **Picks, proposals and seeds key on path, never stem** — six stems in one draw named two files each, and 161 models returned 154 answers; stem keys are read only as a fallback for exports written before that fix, and `merge` refuses a stem that is ambiguous in the draw. Each page exports `confirmed-<mode>-<batch>.json` carrying its own `run` field, because four passes downloading one filename are indistinguishable once saved. A manifest may carry a `note` that becomes the set's description, for a set that is not a random draw. LEARNINGS 2026-09-09, 2026-09-10. |
| `part_probe.py` | Can SigLIP see part-ness? Zero-shot complete-model vs part/accessory margins over the cached embeddings — no rendering, one text embed and a matmul. Structural pseudo-label cohorts (Standalone-Weapons dirs, `_L`/`_R` stems, Environment trees, DM Stash `*Body*`); prints the cohort table, floor sweep and the boundary models. On embed-cache512: ~90% part recall at 0% on handless bodies and 0.4% true character FPs. LEARNINGS 2026-09-03. |
| `text_tokens.py` | Characters vs tokens for `/query`'s text budget (issue #5) — the figures `surface.md` and `src/api.py` cite, where they can be re-run instead of re-typed. Needs only the processor and config from the local HF cache, no weights and no GPU; derives the budget through `query_budget` rather than restating it. Exists because a relayed "100 CJK characters are 52 tokens" was measured from a *spaced* sample and put a query that raises inside a budget it is nearly twice: unspaced, the filer's own case, 100 ideographs are 101 tokens. LEARNINGS 2026-09-08. |
| `renderer_pilot/elev_check.py` | Is the −20° elevation ring earning its half of the cache? Re-ranks the pilot's 12 queries from the live embed-cache512 rows under view subsets sliced from the cached matrix — 16v, each ring alone, alternate azimuths — same GLM judge, delta-judging only newly surfaced pairs. Dropping the −20° ring costs −0.005 nDCG@20, inside pilot noise; halving by azimuth is worse (−0.011). LEARNINGS 2026-09-01, "Elevation rings, finally measured". |
| `glm_vlm.py` | `gemini_vlm.py` for any OpenRouter vision model (`OR_MODEL`, `OR_TAG`, `OR_PROVIDER` pin, `OR_DEADLINE`): same 44 labels, same sheets, same tables. GLM-5.3-Flash low is +2 as the arbiter tier at $0.02/run; max effort loops on ambiguous shapes; gpt-5.6-luna is net −2. LEARNINGS 2026-08-30. |
| `f3d_arbiter.py` | The arbiter on **f3d** tiles instead of the production Open3D tiles, crossed with presentation: numbered contact sheet ("grid") vs six captioned images ("solo"), 256/512/1024, gemini-3.5-flash and an OpenRouter model. f3d tiles cost every model 3–7 of 44; solo helps GLM (+3) and not gemini. |
| `o3d_solo.py` | The cell f3d_arbiter leaves open: production tiles presented solo. GLM low solo@512 is +3 → 41/44 at $0.06/run, one short of production gemini. Renders the tiles once via `rig.pose_sheet_tiles`. |
| `description_panel/` | Free-text description of one model under panels (temperature vs viewset diversity), effort, renderer, and 20× repeats at temperature 0. Own README. |
...
| `capture_vertex_verdicts.py` | The tri-state pass-2 pre-ship check (docs/archive/tri-state-pass-2.md §C2): captures gemini-3.5-flash's real refusal envelopes — a `maxOutputTokens: 1` MAX_TOKENS body and a strict-safety SAFETY block — and reports what `_ask_gemini`'s 200-body split classifies each as. Two paid calls; raw bodies to `eval/out/vertex-verdicts/`, with the shapes pinned durably in `tests/test_pose.py::test_the_captured_vertex_envelopes_classify_as_designed`. Found MAX_TOKENS arrives with `parts: [{"text": ""}]` (the unparseable lane), not the documented no-parts husk. Auth is gcloud ADC. |
...
| `compose_bounds.py` | The two halves of the 2026-08-27 composition change, and the re-runnable home of the figures `src/query.py`'s `rank` docstring cites. `--floor-set` scores a phrase against a real cache and reports the floor set against the ten-row default the API carried before that day: on embed-cache512, `fantasy character` at floor 0.1 gives **875 rows with no count and 10 under the old default**, which is the regression the change exists to prevent. `--orders` needs no cache and answers why the composition order is only observable in `matched`: floor-then-count and count-then-floor select the **same rows for every input** (0 divergences in 20000 tie-forced cases — the floor tests the sort key, so the floor set is a prefix of the order), so a contract test asserting rows cannot see the order at all. CPU by design; these are counts, and a resident `serve_api.py` usually holds the 4060. |
