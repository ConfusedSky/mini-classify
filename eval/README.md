...
| `pose_label_expand.py` | Widens the labelled set: samples the walk excluding every path already labelled (and, with `--exclude`, an earlier batch), renders six-tile sheets, builds the labelling page, and merges confirmed picks into `../up_axis_labels.json` under a new set name. Two page modes because the questions cost their reader differently — `--triage` asks only *model or loose part?* (roughly 40% of this collection is hands, blades and wings with no upright), then the axis pass runs over what survives with a proposal pre-selected. What lands in the labels file is what the human pressed. LEARNINGS 2026-09-09. |
