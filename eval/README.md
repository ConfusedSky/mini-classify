...
| `overlap_spike.py` | The most-cited spike in the refactor set: does a renderer child process feeding SigLIP through a bounded queue saturate the 4060? Three modes — `baseline` (sequential), `overlap` (the ceiling: cached poses, no cycle), `roundtrip` (the real pose→embed dependency graph, child holds up to `--inflight` meshes). Produced the 1.17–1.21× cold-run overlap at ~94% busy, the roundtrip's 1.11× with a three-mesh resident dict, and two structural facts the interfaces note inherits: the parent→child queue is unbounded (the deadlock rule) and the roundtrip *rotated held meshes* — which is why its 1.11× survived I11 intact when the camera-rotation rule for the view renders was measured and reversed (`views_camera_rotation.py`). |
| `views_camera_rotation.py` | I11: must `renderer.views` carry the up-rotation in the camera or in the mesh? Three paths per model × candidate up (the camera trick, production's rotated copy, and a `mesh.rotate` reference), under three arms — post-processing off (byte-stable renders, the real test), indirect light off (attribution), and the production config (scale). The camera trick **fails**: max 75/255 on 53.7% of pixels against a 2/255 repeat floor, because the ambient fill is a world-fixed environment map this Open3D build cannot rotate. The reworked `views` is byte-identical to the reference on 16/18 cases and matches its own repeat noise on the other 2. Also measures what a hidden-then-re-shown geometry costs in pixels (up to 8/255). See LEARNINGS, "camera rotation and the world-fixed fill". |
| `renderer_gil.py` | Splits `_shoot`'s hot line to attribute GIL hold: `render_to_image` holds the GIL ~85–92% of its 36–61 ms, `np.asarray` is free, `Image.fromarray` releases. The Spike-4 result that made the renderer a process rather than a thread; the ModernGL comparison shows ~3–5× less hold, not zero. |
| `parser_gate.py` | Does the numpy STL parser change pose decisions on the labelled set? Runs the production pose path with only the loader changed. The week review found its margin-level claims unattributable (no A/A control, state noise above the effect); the control was then added and the gate passes — and that A/A noise floor is the same renderer nondeterminism `render_determinism.py` later isolated to Filament's post-processing. |
