...
| `compile_pose_flips.py` | The pose-side answer to review R1: the production ensemble path (geometry scores, live candidate tiles, one preprocess) through eager and compiled SigLIP, comparing the up argmax and the `needs_arbiter_margin` gate — the paid-call exposure. 180 models (60 tightest cached margins, 60 nearest the 0.45 gate, 60 control): **1 up flip** at eager margin 2.2e-03 (a plank flipping z to −z), **0 gate flips** — including 4 models whose eager margins sat within the max observed delta of the gate (two at ~4e-03 away). Margin deltas are ~20× the category-side sim deltas (median 1.2e-03, max 1.1e-02 — the min-max in `combine_up_scores` amplifies), and flips still confine to ties. |
