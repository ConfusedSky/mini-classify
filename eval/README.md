...
| `render_determinism.py` | Which stage makes pose resolution nondeterministic (review U2): renders the same loaded mesh's candidate grid repeatedly and hashes the pixels, then runs the tower twice on byte-identical tensors. Answer: **Filament** — ~43% of pixels move by 2–28/255 on every model tried, the tower is bit-deterministic (0.0 delta), margins spread up to 4e-02 across repeats, and one tight-margin model flipped its up *pick* between identical passes. Needs the census from `compile_pose_flips.py`. |
