...
| `embed_store` | numpy, `pose`, `cachedir`, `identity` | **torch** | the read side of what `done` writes. `cluster_models.py` clusters cached vectors and never loads SigLIP; keeping torch out of this module is what lets it stay that way |
