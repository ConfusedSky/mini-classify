...
src/collection.py    the loaded cache an API query answers against: matrix,
                     poses, scope resolution                     [numpy only]
...
| `collection` | numpy, `pose`, `cachedir`, `embed_store`, `identity` | **torch, open3d, fastapi** | the loaded cache an API request answers against, and the seam that keeps the server out of `src/`: it holds no HTTP types and the API layer imports *it*, never the reverse. The rule it enforces on itself is stronger than an import list — **after `load` it touches the filesystem for nothing but a single `stat` per scope**, because the collection lives on spinning exfat where a cold walk is ~32 s and two processes walking one platter contend for the head. Per-file identities and display names are computed once at load, and `resolve` matches precomputed path tuples; `tests/test_collection.py` fails the suite if a scope reaches `iterdir`, `glob` or `os.walk` |
