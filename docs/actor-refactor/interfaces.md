...
[docs/archive/reviews/2026-08-14-interfaces.md](../archive/reviews/2026-08-14-interfaces.md)
...
src/pose.py          math + Pose + caches + the camera geometry both sides
                     share (view_angles, rotation_to_z_up)
...
| `pose` | numpy, PIL, `identity`; **open3d only inside `up_axis_scores`** | torch, **any other `src/` module** | the standing rule, unchanged by the move: `pose` is the leaf both sides import (the child for `up_axis_scores`, the Poser for `combine_up`, `messages` for `Pose`), so it must depend on nothing in the pipeline. Living in `src/` makes it a sibling of its importers, not a peer that may import back. open3d is *deferred* because it was charging 2596 modules to every light consumer — `embed_store` → `cluster_models` loaded a rendering library to read `.npy` files. It qualifies under the signature-implies-dependency test above, and **only `up_axis_scores` does**: any new open3d user here must take a mesh, or the import moves to the caller and the rule is broken without the deferred import moving at all. `import src.pose` adds 196 modules, not 2596. `view_angles` and `rotation_to_z_up` moved here from `renderer` (2026-08-19) and are pure numpy — the second was two open3d calls (`Rx(π)` and Rodrigues) that never touched a mesh, so the rendering library was never the point; it keeps a table of the six exact matrices rather than computing them, because that function decides the pixels of every non-`+Z` render and Rodrigues lands ~5e-17 off in the entries that should be zero. The table was transcribed from open3d's construction, noise included, until the 2026-08-31 rebuild regenerated every embedding it had to stay bit-identical to (docs/cache-rebuild.md §1); `identity.RECIPE_VERSION` is what makes a change to it visible in the key now |
...
| `query` | numpy, stdlib | **torch, `done`, anything holding a model** | the query path both `test_categories.py` and the API server (docs/api/surface.md) call, and the reason it is a module rather than a REPL function: two consumers, one formula. `pool_sims` lived in `done` until 2026-08-19, which meant a tool wanting to pool an array imported the pipeline's terminal stage — torch, csv, the transport — to get it. The dependency runs the other way now: `done` imports the pooling it scores with, because scoring is a leaf and the stage that scores is its consumer. Text embedding stays the caller's (it hands in a `(dim, n_texts)` matrix), which is what keeps torch out: `import src.query` adds 135 modules, all numpy, with torch and open3d absent from `sys.modules` — `tests/test_query.py` asserts that absence in a subprocess so the rule fails loudly rather than drifting |
| `collection` | numpy, `pose`, `cachedir`, `embed_store`, `identity` | **torch, open3d, fastapi** | the loaded cache an API request answers against, and the seam that keeps the server out of `src/`: it holds no HTTP types and the API layer imports *it*, never the reverse. The rule it enforces on itself is stronger than an import list — **after `load`, `pose_of` and `hit` do no I/O at all and a scope costs a handful of `lstat`s bounded by the scope path's depth**, never by the size of the collection. Not for the reason an earlier revision gave — it cited a ~32 s cold walk borrowed from another repo's spinning drive, where this library is ext4 on an SSD and walks in 0.07 s — but because request cost is then independent of the storage: the library may yet move to an HDD, and this budget holds there unchanged, where "a walk would be affordable" was a fact about today's disk. `n_scanned` being a stable claim about the *index* rather than about the tree right now is the second, speed-independent reason. Per-file identities, render keys and display names are computed once at load, and `resolve` matches precomputed path tuples. `tests/test_collection.py` asserts those as syscall *budgets*: an earlier guard patched four walk functions instead, and missed `hit` resolving a path nine times per result — 89 syscalls for a top-10 query (review, 2026-08-19). Note the budget is not justified by this volume being slow — it is not, it is ext4 on an SSD — but by the cost being the same on one that is |
| `api` | fastapi, pydantic, numpy, `collection`, `query` | **torch, open3d, anything in `src/` importing it back** | the HTTP surface, and the top of the stack: it may import the library, nothing in the library may import it. Deliberately thin — every semantic decision lives in `collection` (scoping, the hit shape) or `query` (pooling, robust z), so this module is request parsing, one lock and a status-code mapping. torch is absent because the text pass arrives as a *callable* (`ServerState.embed`), which is also what lets `tests/test_api.py` exercise every route with no GPU. Handlers are `def`, never `async def` — the GPU forward would stall the event loop, where Starlette's threadpool absorbs a sync one; and every handler binds `state.collection` once, since reading it twice can straddle a `/reload` and index a new matrix with rows resolved against the old |
| `embed_store` | numpy, `pose`, `cachedir`, `identity`, `naming` | **torch** | the read side of what `done` writes. `cluster_models.py` clusters cached vectors and never loads SigLIP; keeping torch out of this module is what lets it stay that way. `naming` (stdlib-only) is for the manifest loader, which must apply the walk's `skip()` vocabulary or resurrect models a filter change removed |
